...
class ShowCase (builder: ShowCaseBuilder){
        ...
        if (mTargetView != null && mArrowPositionList.size <= 1) {
            ...
            handler.postDelayed({
                ...
                if (isVisibleOnScreen(target)) {
                    ...
                    addShowCaseMessageViewDependingOnTargetView(target, showCasMessageViewBuilder!!, backgroundDimLayout)
                } else {
                    dismiss()
                }
     ...
     */
    private fun addShowCaseMessageViewDependingOnTargetView(targetView: View?, showCasMessageViewBuilder: ShowCaseMessageView.Builder, backgroundDimLayout: RelativeLayout?) {
        if(targetView==null) return
        ...
        val showCaseMessageView = showCasMessageViewBuilder.targetViewScreenLocation(RectF(
        ...
        backgroundDimLayout?.addView(AnimationUtils.setAnimationToView(showCaseMessageView, animation), showCaseParams)
    }

    /**
     * This function creates a ShowCaseMessageView and it is set on the center of the layout passed by param
     */
    private fun addShowCaseMessageViewOnScreenCenter(showCasMessageViewBuilder: ShowCaseMessageView.Builder, backgroundDimLayout: RelativeLayout?) {
        val showCaseParams = RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, RelativeLayout.LayoutParams.WRAP_CONTENT)
